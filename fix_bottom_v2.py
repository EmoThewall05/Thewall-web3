...
assert content.count(old_sub) == 1, f"Chains text: {content.count(old_sub)}"
content = content.replace(old_sub, new_sub, 1)

# 2. Open wrapper before SIGN UP button
...
# 3. SmartWalletConnect button-feel
...
# 5. Narrow Security box + close wrapper right after its final </div>
...
# 6. Close wrapper: right after CCSS COMPLIANT div's closing divs (2 closes: inner flex row + security box itself)
old_close = "<div>✅<br/>CCSS<br/>COMPLIANT</div>\n            </div>\n          </div>\n          </div>}"
assert content.count(old_close) == 1, f"Close: {content.count(old_close)}"
...
print("[OK] All 6 changes applied successfully")
