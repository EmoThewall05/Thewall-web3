...
export default function TheWall() {
  ...
  const solanaConnectedWallets = []; // TEMP debug
  ...
  const handleSend=async()=>{
    ...
    try {
      if(sendChain==='SOL'){
        ...
        const connection = new Connection(`https://solana-mainnet.g.alchemy.com/v2/${process.env.NEXT_PUBLIC_SOLANA_KEY}`)
    ...
    <div className={styles.loginWrap}>
      <div className={styles.loginCard}>
        ...
        {loginStep==='home'&&<div className="fade-up-1">
          ...
          {/* Reown "SIGN UP / LOGIN" button removed from the login screen to stop
              confusion with Smart Wallet login — external wallet connect is still
              available post-login via handleConnectWallet, just not surfaced here. */}
  ...
  return (
    <div className={styles.dashWrap} style={{paddingBottom:70}}>
      ...
      {sendOpen&&<div className={styles.searchOverlay} onClick={()=>setSendOpen(false)}><div className={styles.searchModal} onClick={e=>e.stopPropagation()}>
        ...
        {sendTab==='send'&&<div>
          <div style={{marginBottom:12}}><div style={s.label}>SELECT CHAIN</div><div style={{display:'flex',gap:8,flexWrap:'wrap',padding:'10px 4px'}}>{SEND_CHAINS.map(c=><button key={c.id} onClick={()=>setSendChain(c.id)} title={c.id} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:4,width:58,padding:'8px 4px',border:'1px solid',borderColor:sendChain===c.id?c.color:'var(--border)',borderRadius:14,background:sendChain===c.id?`radial-gradient(circle at 50% 30%, ${c.color}33, var(--bg3))`:'var(--bg3)',boxShadow:sendChain===c.id?`0 0 14px ${c.color}66, inset 0 0 8px ${c.color}22`:'none',cursor:'pointer',transition:'all 0.2s'}}><span style={{fontSize:'1.15rem',lineHeight:1}}>{c.label.split(' ')[0]}</span><span style={{fontSize:'0.55rem',fontWeight:700,color:sendChain===c.id?c.color:'var(--text-muted)',...s.mono,letterSpacing:'0.02em'}}>{c.id}</span></button>)}</div></div>
        ...
        {sendTab==='receive'&&<div style={{textAlign:'center'}}>
          ...
          :
          <div style={{marginBottom:16}}><div style={{...s.label,marginBottom:8}}>SOLANA</div><div style={{padding:'14px',background:'var(--bg2)',border:'1px solid rgba(153,69,255,0.4)',borderRadius:10,fontSize:'0.72rem',color:'#9945ff',wordBreak:'break-all',...s.mono,lineHeight:1.6}}>{user?.solAddress || 'No Solana address'}</div><button onClick={()=>navigator.clipboard.writeText(user?.solAddress || '')} style={{marginTop:10,padding:'10px 20px',background:'var(--bg3)',border:'1px solid rgba(153,69,255,0.4)',borderRadius:8,color:'#9945ff',...s.mono,fontSize:'0.8rem',cursor:'pointer'}}>📋 Copy SOL</button></div>
          }
        </div>}
      </div></div>}
